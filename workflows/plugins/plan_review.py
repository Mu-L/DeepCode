...
class PlanReviewPlugin(InteractionPlugin):
    ...
    def __init__(self, enabled: bool = True, config: Optional[Dict] = None):
        ...
        self._max_modification_rounds = config.get("max_modification_rounds", 3) if config else 3
    ...
    async def process_response(
        ...
        if action == "confirm":
            ...
            if modification_round > self._max_modification_rounds:
                self.logger.warning(f"Max modification rounds ({self._max_modification_rounds}) reached")
                context["plan_approved"] = True
            ...
            try:
                modified_plan = await self._modify_plan(
                    context.get("implementation_plan", ""),
                    feedback,
                    context
                )
