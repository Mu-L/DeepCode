...
class PluginRegistry:
    ...
    async def run_hook(
        ...
        self.logger.info(f"Running hook {hook_point.value} with {len(plugins)} plugin(s)")
            ...
            try:
                ...
                if self._interaction_callback and task_id:
                    try:
                        ...
                        self.logger.warning(f"Plugin '{plugin.name}' interaction timed out")
                        context = await plugin.on_timeout(context)
                else:
                    # No callback - auto-skip non-required interactions
                    if not interaction.required:
                        self.logger.info(f"No callback, auto-skipping plugin '{plugin.name}'")
                        context = await plugin.on_skip(context)
                    else:
                        raise RuntimeError(
